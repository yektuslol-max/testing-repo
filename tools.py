...
def calculator(expression: str) -> str:
    ...
    try:
        ...
        if not all(c in safe_chars for c in expression):
            result_str = "Error: Invalid characters in expression"
        else:
            result_str = str(eval(expression, {"__builtins__": {}}, {}))
    except Exception as e:
        result_str = f"Error evaluating expression: {str(e)}"

    update_current_span(input={"expression": expression}, output=result_str)
    return result_str
...
def get_current_time() -> str:
    ...
    update_current_span(output=now)
    return now
